...
exports.verifyPayment = function (payment, cb) {
	...
	function checkReceipt(error, result) {
		...
		if (payment.hasOwnProperty('packageName') && payment.packageName !== result.bundle_id) {
			return cb(new Error('Wrong bundle ID: ' + payment.packageName + ' (expected: ' + receipt.bundle_id + ')'));
		}
